...
export function answerAdviser(question: string, context: AdviserContext) {
  ...
  if (/call|writing|presentation|英語授業|英語.*自動/.test(normalized)) return "1年生の英語必修は、CALL・Writing・Presentationを自動登録します。並行クラスがある授業は、時間割と重複しないクラスをそれぞれ1つ選びます。";
  ...
  if (/再履修|未取得|落とした|取れなかった/.test(normalized)) return "学期末に未取得だった授業は再履修候補として残ります。また、途中の学年から始めた場合、修得済み科目の選択でチェックしなかった過去の必修も再履修としてオレンジ色で表示し、時間割へ自動登録します。対象学年を過ぎても再登録できます。";
  if (/コース選択|情報工学コース|データサイエンスコース/.test(normalized)) return `コースは2年後期のボス撃破後、3年1〜2タームへ進むときに選択します。未選択中も、両コースで必要になる最低基準としてDS専門12・情報工学専門6単位を確認でき、情報工学専門は2年次から履修できます。選択後は、情報工学コースがDS基礎8・情報工学基礎13・DS専門12・情報工学専門16単位以上、データサイエンスコースがDS基礎12・情報工学基礎9・DS専門22・情報工学専門6単位以上になります。現在は${context.studyCourse ? `「${studyCourseLabels[context.studyCourse]}」を選択中` : "未選択"}です。`;
  ...
  if (/専門科目|専門.*104|共通専門基礎/.test(normalized)) return context.studyCourse === "dataScience"
    ...
    : context.studyCourse === "information"
      ...
      : "専門科目は合計104単位です。共通専門基礎29・共通専門10単位以上に加え、コース未選択中も最低基準としてDS専門12・情報工学専門6単位を表示します。情報工学専門は2年次から履修でき、3年進級時のコース選択後に各コース固有の最低値へ切り替わります。";
